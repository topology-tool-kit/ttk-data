...
impactvti = XMLImageDataReader(registrationName='hvi.vti', FileName=['hvi.vti'])

impactvti.Set(
    PointArrayStatus=[f"{i:04d}" for i in range(501, 0, -1)],
    TimeArray='None',
)


# create a new 'TTK TrackingFromFields'
tTKTrackingFromFields1 = TTKTrackingFromFields(registrationName='TTKTrackingFromFields1', Input=impactvti)
...
axial_velocity = Threshold(registrationName='Threshold2', Input=tTKTrackingFromFields1)
axial_velocity.Set(
    Scalars=['CELLS', 'ax'],
    LowerThreshold=-27.0,
...
# create a new 'Threshold'
duration = Threshold(registrationName='Threshold4', Input=axial_velocity)
duration.Set(
...
# create a new 'Threshold'
minima = Threshold(registrationName='Threshold3', Input=duration)
minima.Scalars = ['CELLS', 'CriticalType']
minima.LowerThreshold = 0.0
minima.UpperThreshold = 0.0

# create a new 'Threshold'
xBoxOrigin = Threshold(registrationName='Threshold8', Input=minima)
xBoxOrigin.Set(
    Scalars=['CELLS', 'bx'],
...
# create a new 'Threshold'
yBoxOrigin = Threshold(registrationName='Threshold9', Input=xBoxOrigin)
yBoxOrigin.Set(
    Scalars=['CELLS', 'by'],
    LowerThreshold=60.0,
...
def export_crater_profile(source, filename="crater_profile.csv", bin_size=2):
    ...
    np.savetxt(filename, np.column_stack([centers, -counts]),
               delimiter=",", header="X,Counts_neg",
               comments="", fmt="%.6g")
...
def export_ejection_angle(source, filename="ejection_angle.csv"):
    ...
    np.savetxt(filename, np.column_stack([axial, angle]),
               delimiter=",", header="axial_velocity,ejection_angle",
               comments="", fmt="%.6g")
...
SaveData("debrisStat.vtu", yBoxOrigin)
